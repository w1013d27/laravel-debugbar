...
class LaravelDebugbar extends DebugBar
{
    ...
    public function boot()
    {
            ...
            if($this->isFirstBoot) {
                ...
                (function(){
                   foreach($this->aggregates as $index=> $aggregate){
                      if($aggregate->getName() !== 'log' && $aggregate->getName() !== 'swiftmailer_logs'){
                         unset($this->aggregates[$index]); 
                      } 
                   }
                   $this->messages =[];
                })->bindTo($this['messages'],MessagesCollector::class)();
            }
        }
        ...
        if ($this->shouldCollect('events', false) && isset($this->app['events'])) {
            try {
                ...
                if ($this->isFirstBoot) {
                    ...
                    $this['events']->__construct($startTime);
                    (function () {
                        $this->startedMeasures = array();
                        $this->measures = array();
                    })->bindTo($this['events'], EventCollector::class)();
                }
            } catch (\Exception $e) {
                $this->addThrowable(
                    new Exception(
        ...
        if ($this->shouldCollect('views', true) && isset($this->app['events'])) {
            try {
                ...
                if ($this->isFirstBoot) {
                    ...
                    $this['views']->__construct($collectData);
                }
            } catch
            (\Exception $e) {
                $this->addThrowable(
            ...
            try {
                if ($this->hasCollector('messages')) {
                    if ($this->isFirstBoot) {
                        ...
                        (function () {
                            foreach ($this->aggregates as $aggregate) {
                                if ($aggregate->getName() === 'log') {
                                    (function () {
                                        $this->messages = [];
                                    })->bindTo($aggregate, get_class($aggregate))();
                                }
                            }
                        })->bindTo($this['messages'], MessagesCollector::class)();
                    ...
                    if ($this->isFirstBoot) {
                        ...
                        (function () {
                            $this->records = [];
                        })->bindTo($this['monolog'], MonologCollector::class)();
                    }
                }
            } catch (\Exception $e) {
                $this->addThrowable(
                    ...
                    $db->listen(
                        function (
                            ...
                            if (!$threshold || $time > $threshold) {
                                $debugbar['queries']->addQuery((string)$query, $bindings, $time, $connection);
                            }
                        }
                    );
        ...
        if ($this->shouldCollect('models', true)) {
            if($this->isFirstBoot) {
                ...
                (function(){
                    $this->count= 0;
                    $this->models=[];
                })->bindTo($this['models'],get_class($this['models']))();
            }
        }
        ...
        if ($this->shouldCollect('livewire', true) && $this->app->bound('livewire')) {
            if($this->isFirstBoot) {
                ...
                (function(){
                    $this->data =[];
                })->bindTo($this['livewire'],get_class($this['livewire']))();
            }
        }
        ...
        if ($this->shouldCollect('mail', true) && class_exists('Illuminate\Mail\MailServiceProvider')) {
            try {
                if($this->isFirstBoot) {
                    ...
                    (function(){
                        $this->messagesLogger->clear();
                    })->bindTo( $this['swiftmailer_mails'],get_class($this['swiftmailer_mails']))();
                }
                if (
                    ...
                    if($this->isFirstBoot) {
                        ...
                        (function(
                            ...
                            foreach ($this->aggregates as $aggregate){
                               if($aggregate->getName() === 'swiftmailer_logs'){
                                   (function(){
                                       $this->messages =[];
                                   })->bindTo($aggregate,SwiftLogCollector::class);
                               } 
                            }
        ...
        if ($this->shouldCollect('gate', false)) {
            try {
               if($this->isFirstBoot) {
                   ...
                   (function(){
                       $this->messages=[];
                   })->bindTo( $this['gate'],'Barryvdh\Debugbar\DataCollector\GateCollector')(); 
               }
    ...
    public function modifyResponse(Request $request, Response $response)
    {
        ...
        if ($this->shouldCollect('symfony_request', true)) {
            try {
                $reqId = $this->getCurrentRequestId();
                if($this->hasCollector('request')){
                    $this['request']->__construct($request, $response, $sessionManager, $reqId);
                }else{
